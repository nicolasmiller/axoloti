...
public class PatchController {
    ...
    public void disconnect(InletInstanceView ii) {
        if (!patchView.isLocked()) {
            ii.getInletInstance().disconnect();
        } else {
            Logger.getLogger(PatchController.class.getName()).log(Level.INFO, "Can't disconnect: locked!");
        }
    }
    
    public void disconnect(OutletInstanceView oi) {
        if (!patchView.isLocked()) {
            oi.getOutletInstance().disconnect();
        } else {
            Logger.getLogger(PatchController.class.getName()).log(Level.INFO, "Can't disconnect: locked!");
        }
    }

    public void AddConnection(InletInstanceView il, OutletInstanceView ol) {
        if (!patchView.isLocked()) {
            patchModel.AddConnection(il.getInletInstance(), ol.getOutletInstance());
        } else {
            Logger.getLogger(PatchController.class.getName()).log(Level.INFO, "can't add connection: locked");
        }
    }

    public void AddConnection(InletInstanceView il, InletInstanceView ol) {
        if (!patchView.isLocked()) {
            patchModel.AddConnection(il.getInletInstance(), ol.getInletInstance());
        } else {
            Logger.getLogger(PatchController.class.getName()).log(Level.INFO, "Can't add connection: locked!");
        }
    }
