...
public class PDialComponent extends PCtrlComponentAbstract {
    ...
    public PDialComponent(double value, double min, double max, double tick) {
        super();
// TODO
...
//        SetupTransferHandler();
    ...
    private int getScreenY(PInputEvent e) {
        Point2D p2d = e.getCanvasPosition();
        Point p = new Point((int) p2d.getX(), (int) p2d.getY());
        SwingUtilities.convertPointToScreen(p, this.axoObjectView.getCanvas());
        return p.y;
    }

    private int getScreenX(PInputEvent e) {
        Point2D p2d = e.getCanvasPosition();
        Point p = new Point((int) p2d.getX(), (int) p2d.getY());
        SwingUtilities.convertPointToScreen(p, this.axoObjectView.getCanvas());
        return p.x;
        ...
        if (isEnabled()) {
            ...
            if ((MousePressedBtn == MouseEvent.BUTTON1)) {
                ...
                if (MainFrame.prefs.getMouseDialAngular()) {
                    int y = (int) getScreenY(e);
                    int x = (int) getScreenX(e);
                    int radius = (int) Math.min(getBounds().width, getBounds().height) / 2 - layoutTick;
                    ...
                    v = value + t * ((int) Math.round((MousePressedCoordY - getScreenY(e))));
                }
                setValue(v);
                e.setHandled(true);
            }
    ...
    protected void mousePressed(PInputEvent e) {
        if (!e.isPopupTrigger()) {
            if (isEnabled()) {
                grabFocus();
                MousePressedCoordX = (int) getScreenX(e);
                MousePressedCoordY = (int) getScreenY(e);
