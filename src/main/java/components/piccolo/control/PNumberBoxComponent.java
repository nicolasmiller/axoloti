...
public class PNumberBoxComponent extends PCtrlComponentAbstract {
    ...
    public PNumberBoxComponent(double value, double min, double max, double tick) {
        this(value, min, max, tick, 50, 16);
    }

    public PNumberBoxComponent(double value, double min, double max, double tick, int hsize, int vsize) {
...
//        SetupTransferHandler();
    ...
    protected void mouseDragged(PInputEvent e) {
        if (isEnabled() && dragging) {
            ...
            if ((MousePressedBtn == MouseEvent.BUTTON1)) {
                ...
                v = value + t * (MousePressedCoordY - e.getCanvasPosition().getY());
                if (robot == null) {
                    try {
    ...
    protected void mousePressed(PInputEvent e) {
        if (!e.isPopupTrigger()) {
            grabFocus();
            if (isEnabled() && (e.getCanvasPosition().getX() >= getWidth() - rmargin - htick * 2)) {
                dragging = false;
                if (e.getCanvasPosition().getY() > getHeight() / 2) {
                    hiliteDown = true;
                ...
                dragging = true;
                MousePressedCoordX = (int) e.getCanvasPosition().getX();
                MousePressedCoordY = (int) e.getCanvasPosition().getY();
