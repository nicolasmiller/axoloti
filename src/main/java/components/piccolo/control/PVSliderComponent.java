...
public class PVSliderComponent extends PCtrlComponentAbstract {
    ...
    public PVSliderComponent(double value, double min, double max, double tick) {
        this.max = max;
        this.min = min;
        this.value = value;
        this.tick = tick;
...
//        SetupTransferHandler();
    ...
    protected void mouseDragged(PInputEvent e) {
        if (isEnabled()) {
            double v = value + tick * ((int) Math.round((py - e.getCanvasPosition().getY())));
            robotMoveToCenter(e);
            setValue(v);
        }
    }
    ...
    protected void mousePressed(PInputEvent e) {
        if (!e.isPopupTrigger()) {
            grabFocus();
            px = (int) e.getCanvasPosition().getX();
            py = (int) e.getCanvasPosition().getY();
